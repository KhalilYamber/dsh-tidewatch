 ...
 *
 * 价格时效（官方 2026-09-10 新闻 + 定价页）：
 *   - 2026-09-10 04:00 UTC 起：DeepSeek-V4.1-Flash 上线，模型名 `deepseek-flash`，
 *     新价生效；旧名 deepseek-v4-flash / deepseek-v4-flash-vision-exp 仍可调用，
 *     但由 V4.1-Flash 服务并按 Flash 新价计费。
...
/** V4.1-Flash 新价生效（2026-09-10 04:00 UTC）；旧 Flash 名此前按旧 V4-Flash 价。 */
export const FLASH_V41_BOUNDARY = '2026-09-10T04:00:00Z'

/** deepseek-v4-pro 请求改按 Flash 价计费（2026-09-14 04:00 UTC）。 */
export const PRO_TO_FLASH_BOUNDARY = '2026-09-14T04:00:00Z'
 ...
 *
 * Flash 条目：
 *   - 当前档 = V4.1-Flash 新价（2026-09-10 04:00 UTC 起）
 *   - beforeFlashV41 = 旧 V4-Flash 价（新价生效前的历史正确性）
 * Pro 条目：
 *   - 当前档仍为 Pro 官方价；routeToFlashAfter 起改按 Flash 新价计费
 */
export const DEFAULT_PRICE_TABLE = {
  models: {
    ...
    // 旧名仍可调用：2026-09-10 后由 V4.1-Flash 服务并按 Flash 新价计费。
    'deepseek-v4-flash': {
      cacheHit: 0.003,
      cacheMiss: 0.15,
      output: 0.6,
    ...
    'deepseek-v4-pro': {
      ...
      // 2026-09-14 04:00 UTC 起：请求路由到 V4.1-Flash，按 Flash 价计费。
      routeToFlashAfter: {
        cacheHit: 0.003,
        cacheMiss: 0.15,
        output: 0.6,
...
/**
 * 为一次用量挑选价格档位：
 *   - 峰谷时代前（2026-08-16 16:00 UTC）→ 当时基础价
 *   - V4.1-Flash 新价前的 Flash 条目 → beforeFlashV41
 *   - 2026-09-14 后的 pro → routeToFlashAfter（路由到 Flash 计价）
...
export function tierFor(entry, atMs) {
  ...
  const pickTier = table => {
    if (Number.isFinite(atMs) && atMs < Date.parse(LEGACY_BASE_BOUNDARY)) {
      const lb = table.legacyBase
      return lb === undefined ? asTier(table) : asTier(lb)
    }
    if (isPeakHour(atMs)) {
      const p = table.peak
      return p === undefined ? asTier(table) : asTier(p)
    }
    const off = table.offPeak
    return off === undefined ? asTier(table) : asTier(off)
  }
  if (Number.isFinite(atMs)) {
    // V4.1-Flash 新价前：旧 Flash 名按旧 V4-Flash 价。
    if (base.beforeFlashV41 !== undefined && atMs < Date.parse(FLASH_V41_BOUNDARY)) {
      return pickTier(base.beforeFlashV41)
    }
    // 2026-09-14 起：pro 请求路由到 Flash，按 Flash 价计费。
    if (base.routeToFlashAfter !== undefined && atMs >= Date.parse(PRO_TO_FLASH_BOUNDARY)) {
      return pickTier(base.routeToFlashAfter)
    }
  }
  return pickTier(base)
}
