...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const FX_DEFAULT = 7.2
    const FX_KEY = 'dsh-tidewatch:fx'
    ...
    const css = [
      ...
      '.tw-fx{display:flex;align-items:center;gap:8px;margin-top:8px;padding:0 2px;font-size:11px;color:var(--dsw-alias-label-tertiary,#9b9b9b)}',
      '.tw-fx input{width:64px;font:inherit;font-size:12px;color:var(--dsw-alias-label-primary,#e8e8e8);background:var(--dsw-alias-bg-layer-1,transparent);border:1px solid var(--dsw-alias-border-l1,#444);border-radius:6px;padding:2px 6px;outline:none;text-align:right}',
      '.tw-fx input:focus{border-color:var(--dsw-alias-state-business-primary,#4d9fff)}',
      ...
      useEffect(() => {
        ...
        function measure() {
          ...
          raf = requestAnimationFrame(() => {
            ...
            const sidebarW = sidebar ? sidebar.getBoundingClientRect().width : 56
    ...
    function TideCard() {
      ...
      const onFxChange = e => {
        ...
        setFx(Number.isFinite(v) && v > 0 ? v : FX_DEFAULT)
        try { localStorage.setItem(FX_KEY, e.target.value) } catch { /* ignore */ }
      }

      // 当前模型：优先取投影 byModel 的首键，缺省 flash。
      const modelKeys = usage?.byModel !== undefined ? Object.keys(usage.byModel) : []
      const modelKey = modelKeys.length > 0 ? priceEntryForModel(modelKeys[0]) : 'deepseek-v4-flash'
      ...
      return el('div', { ref: rootRef, className: 'tw-root', style: { right: pos.right, bottom: pos.bottom } },
        el('button', {
          ...
          el('span', { className: 'tw-cost' }, fmtMoney(costUsd, fx))),
        expanded ? el('div', { className: 'tw-panel' },
          ...
          el('div', { className: 'tw-prices' },
            el('div', { className: 'tw-price-row' },
              ...
              el('span', { className: 'tw-price-val' }, '¥' + (tier.cacheMiss * fx).toFixed(2) + ' / ¥' + (tier.cacheHit * fx).toFixed(2) + ' /百万')),
            el('div', { className: 'tw-price-row' },
              el('span', { className: 'tw-price-label' }, '输出'),
              el('span', { className: 'tw-price-val' }, '¥' + (tier.output * fx).toFixed(2) + ' /百万'))),
          // Token 明细
          ...
          el('div', { className: 'tw-total' },
            ...
            el('span', { className: 'tw-total-val' }, fmtMoney(costUsd, fx))),
          el('div', { className: 'tw-fx' },
            el('span', null, '汇率（美元 → 人民币）'),
            el('input', { type: 'number', step: '0.01', min: '0.01', value: fx, onChange: onFxChange })))
          : null)
    }
